<?php
    ...
    if(isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0){
        ...
        $productId = $_GET['id'];
        $openedRow = getOneCategory($connection, $productId);
        if($openedRow){
            $status = TRUE;
        }else{
    ...
    if($status){
        ...
        if (isset($formData["submit"]) && $formData["submit"] == "Yes, delete") {
            ...
            $deleteStatus = deleteCategory($connection, $productId);
            if($deleteStatus){
                $systemMessage = "Category successfully deleted!!!";
            } else{
    ...
    include_once '../../../views/categories/deleteView.php';
